...
var Agent = {isIE: false};
 ...
 var PointMass = function(cx, cy, mass)
  {
    this.cur = new VectorBlob(cx, cy); 
    this.prev = new VectorBlob(cx, cy); 
    this.mass = mass; 
    this.force = new VectorBlob(0.0, 0.0); 
    this.result = new VectorBlob(0.0, 0.0); 
    this.friction = 0.01; 
  ...
  function ConstraintY(pointMass, y, shortConst, longConst)
  {
    ...
    this.y = y; 
    this.delta = new VectorBlob(0.0, 0.0);
  ...
  var Joint = function(pointMassA, pointMassB, shortConst, longConst)
  {
    ...
    this.delta._set(this.pointMassBPos); 
  ...
  var Blob = function (x, y, radius, numPointMasses)
  {
    ...
    this.draw = function(ctx, scaleFactor)
    {
      ...
      up = new VectorBlob(0.0, -1.0); 
      ori = new VectorBlob(0.0, 0.0); 
      ori._set(this.pointMasses[0].getPos()); 
  ...
  var BlobCollective = function (x, y, startNum, maxNum)
  {
    ...
    this.tmpForce = new VectorBlob(0.0, 0.0); 
    ...
    {
      var minDist = 1000.0, foundIndex = -1, dist, aXbX, aYbY;
      var i;
    ...
    this.setForce = function(force)
    {
      ...
      for(i = 0; i < this.blobs.length; i++)
      {
        ...
        {
          this.blobs[i].setForce(new VectorBlob(0.0, 0.0)); 
          continue; 
        }
...
//    }
  ...
  {
	if(Agent.isIE){
		var canvas = document.createElement('custom:canvas');
	}
	else{
	...
	canvas.height = 400;
	canvas.id = "blob";
	canvas.Identifier = "blob";
	document.body.appendChild(canvas);
	//var context = canvas.getContext('2d');
    var canvas = document.getElementById('blob');
//    if(canvas.getContext == null)
//    {
//      MessageBox.Show("You need Firefox version 1.5 or higher for this to work, sorry."); 
//      return; 
//    }
    ...
    addEvent(document, 'keydown', function(e) {
        ...
        switch (keyCode) {
            // left  
            case 37:
                blobColl.addForce(new VectorBlob(-50.0, 0.0));
                break;

            // up  
            case 38:
                blobColl.addForce(new VectorBlob(0.0, -50.0));
                break;
    ...
    function getMouseCoords(e)
    {
	  ...
	  if(e.pageX) { x = e.pageX; y=e.pageY; }
		  ...
		  if(e.clientX) {x = e.clientX; y = e.clientY;} 
			...
			if('localX' in e) { x = e.localX; y = e.localY; }			
			else {
				if('stageX' in e) { x = e.stageX; y = e.stageY; }
			}
		  }
	  }
    ...
    gravity = new VectorBlob(0.0, 10.0); 
    stopped = false; 
